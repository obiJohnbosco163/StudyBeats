...
export async function analyzeContent(env: unknown, text: string): Promise<ContentAnalysis> {
  const trimmed = truncateText(text, MAX_INPUT_CHARS);
  const system =
    'You are a meticulous study-notes analyst. Respond with ONLY valid JSON matching the requested schema — no markdown fences, no commentary, no extra keys.';
