...
import { recordQueueJobStatus, resolvePoofAppId, } from '../lib/poof-queue.js';
...
function assertValidQueueName(queueName) {
...
function getQueueConfig(queueName) {
    ...
    return queuesConfig.queues.find((queue) => queue.name === validQueueName);
}
...
function verifyInternalQueueDispatch(c) {
    let expectedAppId;
    try {
        expectedAppId = resolvePoofAppId(c.env);
    }
    catch {
...
function resolveTrackingAppId(envelope, env) {
    if (typeof envelope.__poofAppId === 'string' && envelope.__poofAppId)
        return envelope.__poofAppId;
    try {
...
async function safeRecordQueueJobStatus(env, appId, jobId, update) {
    if (!appId || !jobId)
        return;
    try {
...
async function processEnvelope(envelope, rawMessage, env, ctx) {
...
async function markEnvelopeDeadLetter(queueName, payload, env, options) {
...
export async function markQueuePayloadDeadLetter(queueName, payload, env, options = {}) {
    await markEnvelopeDeadLetter(queueName, payload, env, options);
}
async function processMessage(rawMessage, env, ctx) {
    await processEnvelope(unwrapMessage(rawMessage), rawMessage, env, ctx);
}
export async function processQueuePayload(queueName, payload, env, options = {}) {
    const validQueueName = assertValidQueueName(queueName);
    const rawMessage = {
        id: options.id,
...
export async function processQueueBatch(batch, env, ctx) {
    await Promise.allSettled(batch.messages.map(async (message) => {
        try {
            ...
            console.error(`[queue] Failed ${queueName}: ${error instanceof Error ? error.message : String(error)}`);
...
export function registerQueueRoutes(app) {
    ...
    app.post('/__internal/queues/:queueName', async (c) => {
        ...
        try {
            await processQueuePayload(queueName, body.payload, c.env, {
    ...
    app.post('/__internal/queues/:queueName/dead-letter', async (c) => {
        ...
        try {
            await markQueuePayloadDeadLetter(queueName, body.payload, c.env, {
                id: typeof body.id === 'string' ? body.id : undefined,
                attempts: typeof body.attempts === 'number' ? body.attempts : undefined,
                jobId: typeof body.jobId === 'string' ? body.jobId : undefined,
            });
