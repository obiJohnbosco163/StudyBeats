...
export async function generateSummary(env, text) {
    const trimmed = truncateText(text, MAX_INPUT_CHARS);
    const system = 'You are a precise academic summarizer. Respond with ONLY valid JSON — no markdown fences, no commentary.';
...
${trimmed}
"""`;
    const raw = (await chatJson(env, system, user, 1200));
    return {
